...
`_tmp_tracking/` 是 _digested 维护过程中的**临时工作区**。比散落在仓库根目录的零散文件更有组织，但不属于正式的 `_digested/` 内容。

> **原则**：这里的内容在工作 session 期间存在，一旦结论稳定，应写入正式的 `_digested/` 系统，然后从此目录移除。
...
| `_digested_issues/` | _digested 相关的问题追踪（待覆盖的源码区域、需要审查的文档、发现的 gap 等） |

## 使用约定

- 文件名以日期为前缀（如 `2026-07-17-review-agent-core.md`）
