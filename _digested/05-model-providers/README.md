...
purpose: "列出 05-model-providers 目录的计划文档"
...
OpenWiki 支持 9+ 模型提供商，每个 provider 在 `createModel()` 中有独立的模型客户端创建路径，不是抽象工厂模式。提供商解析有明确的回退链。
...
2. 否则，按顺序找第一个有 API key 的提供商：OpenAI → OpenAI-compatible → OpenRouter → Anthropic → Baseten → Fireworks → NVIDIA
...
| Provider | 客户端 | 特点 |
|----------|--------|------|
| **vertex** | `ChatAnthropic` + `AnthropicVertex` client | Google ADC 认证，无需 API key，`GOOGLE_CLOUD_PROJECT` |
...
| **baseten / fireworks / nvidia / openai-compatible** | `ChatOpenAI` + 自定义 baseURL | OpenAI 兼容客户端 |

## 计划文档

| # | 文档 | 说明 |
|---|------|------|
| 1 | `01-provider-configuration.md` | 提供商注册（`PROVIDER_CONFIGS`、`OpenWikiProvider`）、模型列表、env key 定义、回退链 |
| 2 | `02-provider-model-creation.md` | `createModel()` 每个 provider 分支的详细分析（含行号） |
| 3 | `03-chatgpt-oauth-provider.md` | `openai-chatgpt` — OAuth 登录、token 持久化刷新、Codex 后端（546 行） |
| 4 | `04-vertex-ai-provider.md` | `vertex` — Google ADC、`AnthropicVertex` client、`resolveProviderLocation()` |
