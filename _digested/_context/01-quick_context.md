...
| 模式 | 入口 | 做什么 |
|------|------|--------|
| **Code mode**（代码模式） | `openwiki code` | 分析 Git 仓库源码，在 `openwiki/` 目录下生成结构化文档 wiki，可集成 CI/CD 定时更新 |
| **Personal mode**（个人模式） | `openwiki`（默认） | 从 Gmail、Slack、Notion 等数据源摄取内容，在 `~/.openwiki/wiki/` 构建个人知识 brain |
