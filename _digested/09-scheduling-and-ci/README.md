...
purpose: "列出 09-scheduling-and-ci 目录的计划文档"
...
| # | 文档 | 说明 |
|---|------|------|
| 1 | `01-macos-launchagents.md` | macOS LaunchAgent 管理（schedules.ts，918 行）— 创建、列出、删除、cron 表达式解析 |
| 2 | `02-ci-workflows.md` | CI 流水线 — checks.yml（format → lint → build/typecheck/smoke → test → Trivy 安全审计）、示例模板（GitHub Actions、GitLab CI、Bitbucket Pipelines） |
| 3 | `03-scheduled-openwiki-updates.md` | 定时 wiki 更新工作流（openwiki-update.yml）— fork opt-in 机制（`OPENWIKI_ENABLE_SCHEDULED_UPDATE=true`）、OpenRouter + glm-5.2 模型调用 |
